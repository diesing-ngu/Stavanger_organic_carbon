...
resp_centroids <- resp_centroids %>% group_by(ID) %>% summarize(d13C_pm = mean(d13C_pm, na.rm = TRUE))
...
```{r save_files}
write_sf(AoI, here("delta13c", "data", "interim", "AoI.shp"))
writeRaster(predictors, here("delta13c","data", "interim", "predictors_aoi.tif"), overwrite = TRUE)
write_sf(resp, dsn = here("delta13c","data", "interim"), layer = "resp", driver = "ESRI Shapefile")
write.csv(rm_resp, here("delta13c","data", "interim", "rm_resp.csv"), quote = FALSE, row.names = FALSE)
```
