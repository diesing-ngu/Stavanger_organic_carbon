...
Organic carbon stock of the upper 0.1 m of sediment

```{r calc_oc_stocks}
resp_data$OCS_kg_m2 <- (resp_data$TOC_pc/100) * 1000 * resp_data$DBD_g_cm3 * 0.1
```
...
resp_centroids <- resp_centroids %>% group_by(ID) %>% summarize(OCS_kg_m2 = mean(OCS_kg_m2, na.rm = TRUE))
...
```{r save_files}
write_sf(AoI, here("oc_stocks", "data", "interim", "AoI.shp"))
writeRaster(predictors, here("oc_stocks","data", "interim", "predictors_aoi.tif"), overwrite = TRUE)
write_sf(resp, dsn = here("oc_stocks","data", "interim"), layer = "resp", driver = "ESRI Shapefile")
write.csv(rm_resp, here("oc_stocks","data", "interim", "rm_resp.csv"), quote = FALSE, row.names = FALSE)
```
